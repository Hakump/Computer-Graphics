...
export class GrCrane2 extends GrObject{
  constructor(params = {}) {
    let crane2 = new T.Group();
    ...
    /**@type THREE.Shape */
    let base_curve = new T.Shape();
    base_curve.moveTo(-0.75, 0);
    base_curve.lineTo(-0.5, 2);
    base_curve.lineTo(-0.4, 5);
    base_curve.lineTo(-0.2, 5.5);
    base_curve.lineTo(0.2, 5.5);
    base_curve.lineTo(0.4, 5);
    base_curve.lineTo(0.5, 2);
    base_curve.lineTo(0.5, 0);
    base_curve.lineTo(-0.75, 0);
    let base_geom = new T.ExtrudeGeometry(base_curve, exSettings);
    let crane_mat = new T.MeshStandardMaterial({
      color: "yellow",
      metalness: 0.5,
    ...
    let base = new T.Mesh(base_geom, crane_mat);
    crane2.add(base);
    base.translateZ(-1.25);


    let arm_group = new T.Group();
    crane2.add(arm_group);
    arm_group.translateY(4.5);
    let arm_curve = new T.Shape();
    arm_curve.moveTo(-1.5, 0);
    arm_curve.lineTo(-1.5, 0.25);
    ...
    let wire_group = new T.Group();
    arm_group.add(wire_group);
    wire_group.translateX(1);
    let wire1 = new T.Shape();
    wire1.moveTo(0,0);
    wire1.lineTo(0,-3);
    wire1.lineTo(0.01,-3);
    wire1.lineTo(0.01,0);
    wire1.lineTo(0,0);
    let wire2 = new T.Shape();
    wire2.moveTo(0,0);
    wire2.lineTo(0,-3);
    wire2.lineTo(0.01,-3);
    wire2.lineTo(0.01,0);
    wire2.lineTo(0,0);
    let wire1_geom = new T.ExtrudeGeometry(wire1, exSettings);
    let wire2_geom = new T.ExtrudeGeometry(wire2, exSettings);
    let wire_mat = new T.MeshStandardMaterial({
      color: "#888888",
      metalness: 0.6,
      roughness: 0.3
    });
    let wire_1 = new T.Mesh(wire1_geom, wire_mat); wire_1.translateZ(-1.25); wire_1.translateX(-0.25);
    let wire_2 = new T.Mesh(wire2_geom, wire_mat); wire_2.translateZ(-1.25); wire_2.translateX(0.25);
    wire_group.add(wire_1,wire_2);

    let circle_group = new T.Group();
    arm_group.add(circle_group);
    let circleG = new T.Shape();
    circleG.moveTo(-0.25,-0.5);
    circleG.lineTo(-0.25,0.5);
    circleG.lineTo(0.25,0.5);
    circleG.lineTo(0.25,-0.5);
    circleG.lineTo(-0.25,-0.5);
    let circle_geom = new T.ExtrudeGeometry(circleG,exSettings);
    let circle = new T.Mesh(circle_geom, wire_mat);
    circle.translateY(-3); circle.translateZ(-1.25);
    circle_group.add(circle);
    let hang = new T.Shape();
    hang.moveTo(-0.05,-0.5);
    hang.lineTo(-0.05,-0.8);
    hang.lineTo(-0.1,-0.7);
    hang.lineTo(0.05, -0.9);
    hang.lineTo(0.05,-0.5);
    hang.lineTo(-0.05,-0.5);
    let hang_geom = new T.ExtrudeGeometry(hang,exSettings);
    let hang_ = new T.Mesh(hang_geom, wire_mat);
    hang_.translateY(-3); hang_.translateZ(-1.25);
    circle_group.add(hang_);

    super(`Crane2-${craneObCtr++}`, crane2, [
      ["x", -4, 4, 0],
      ["z", -4, 4, 0],
      ["theta", 0, 360, 0],
      ["wire", 1, 3.5, 2],
    ...
    this.whole_ob = crane2;
    this.arm = arm_group;
    this.wire = wire_group;
    this.circle = circle_group;
  ...
  update(paramValues) {
    ...
    this.whole_ob.position.z = paramValues[1];
    this.whole_ob.rotation.y = degreesToRadians(paramValues[2]);
    this.wire.position.x = paramValues[3]; this.circle.position.x = paramValues[3];
    this.arm.rotation.y = degreesToRadians(paramValues[4]);
    this.circle.position.y = paramValues[5];
    let sc = (2.5-paramValues[5])/2.5;
    this.wire.scale.set(1,sc,1);
  }
}

export class back_hoe extends GrObject{
  constructor() {
    ...
    cab_curve.moveTo(-1, 0);
    cab_curve.lineTo(1, 0);
    cab_curve.lineTo(1.2, 0.55);
    cab_curve.lineTo(1, 0.95);
    cab_curve.lineTo(0.25, 0.95);
    cab_curve.lineTo(0, 1.7);
    cab_curve.lineTo(-0.8, 1.7);
    cab_curve.lineTo(-1, 1.4);
    cab_curve.lineTo(-1, 0);
    let cab_geom = new T.ExtrudeGeometry(cab_curve, exSettings);
    let cab = new T.Mesh(cab_geom, excavator_mat);
    cab_group.add(cab);
    ...
    let arm_group = new T.Group();
    cab_group.add(arm_group);
    arm_group.translateY(0.5);
    arm_group.position.x =-0.8;
    let arm_curve = new T.Shape();
    arm_curve.moveTo(-2, 0);
    arm_curve.lineTo(-2.1, -0.15);
    arm_curve.lineTo(-1, -0.5);
    arm_curve.lineTo(0, -0.25);
    arm_curve.lineTo(-0.2, 0);
    arm_curve.lineTo(-1, -0.3);
    arm_curve.lineTo(-2, 0);
    let arm_geom = new T.ExtrudeGeometry(arm_curve, exSettings);
    let arm_mat = new T.MeshStandardMaterial({
      color: "#888888",
      metalness: 0.6,
    ...
    let forearm_group = new T.Group();
    arm_group.add(forearm_group);
    forearm_group.position.x = -1.95;
    let forearm_curve = new T.Shape();
    forearm_curve.moveTo(-2.1, 0);
    forearm_curve.lineTo(-2.1, 0.1);
    forearm_curve.lineTo(0, 0.15);
    forearm_curve.lineTo(0.15, 0);
    forearm_curve.lineTo(-2.1, 0);
    let forearm_geom = new T.ExtrudeGeometry(forearm_curve, exSettings);
    let forearm = new T.Mesh(forearm_geom, arm_mat);
    forearm_group.add(forearm);
    forearm.translateZ(-0.2);
    ...
    let bucket_group = new T.Group();
    forearm_group.add(bucket_group);
    bucket_group.position.x = -2;
    let bucket_curve = new T.Shape();
    bucket_curve.moveTo(0, 0);
    bucket_curve.lineTo(0.1, -0.2);
    bucket_curve.lineTo(0.1, -0.26);
    bucket_curve.lineTo(0.05, -0.36);
    bucket_curve.lineTo(0.0, -0.42);
    bucket_curve.lineTo(-0.1, -0.52);
    bucket_curve.lineTo(-0.4, -0.70);
    bucket_curve.lineTo(-0.7, -0.88);
    bucket_curve.lineTo(-1, -1.02);
    bucket_curve.lineTo(-0.2, -0.26);
    bucket_curve.lineTo(-0.2, 0);
    bucket_curve.lineTo(0, 0);

    //bucket_curve.moveTo(0.25,);
    let bucket_geom = new T.ExtrudeGeometry(bucket_curve, exSettings);
    let bucket = new T.Mesh(bucket_geom, arm_mat);
    bucket_group.add(bucket);
    bucket.translateZ(-0.2);
    ...
    super(`backhoe-${excavatorObCtr++}`, backhoe, [
      ["x", -10, 10, -3],
      ["z", -10, 10, 2],
      ["theta", 0, 360, 0],
      ["spin", 0, 360, 0],
      ["height", 0.2, 1.1, 0.5],
      ["arm rotate", 0, 60, 55],
      ["forearm rotate", 20, 120, 100],
      ["bucket rotate", -135, -15, -50]
    ]);
  ...
  update(paramValues) {
    ...
    this.forearm.rotation.z = degreesToRadians(paramValues[6]) ;
    this.bucket.rotation.z = degreesToRadians(paramValues[7])+ Math.PI / 16;
  }
}
