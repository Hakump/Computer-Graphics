...
export class GrColoredRoundabout_2 extends GrObject{
  /**
   * @param {ColoredRoundaboutProperties} params
   */
  constructor(params = {}) {
    ...
    for (let i = 0; i < section_colors.length; i++) {
      ...
      loader.load("../for_students/teapot.obj", function(suzanne) {
        suzanne.scale.set(0.03, 0.03, 0.03);
        //suzanne.position.set(0,0.6,0);
        suzanne.translateY(0.8);
        suzanne.rotateY(i*Math.PI/2 +Math.PI/4);
        suzanne.translateX(1.1);
        section.add(suzanne);
        cups.push(suzanne);
      });
      section.add(handle);
...
export class GrCarousel extends GrObject {
  ...
  constructor(params = {}) {
    ...
    let horses = [];
    let loader = new OBJLoader();
    //let horseG = new T.BoxGeometry(0.5,0.2,0.5,32,32,32);
    let opole;
    let num_poles = 10;
    let poles = [];
    for (let i = 0; i < num_poles; i++) {
      opole = new T.Mesh(opole_geom, opole_mat);
      //let horse = new T.Mesh(horseG,opole_mat);
      platform_group.add(opole);
      //platform_group.add(horse);
      opole.translateY(1.5); //horse.translateY(1.5);
      opole.rotateY((2 * i * Math.PI) / num_poles); //horse.rotateY((2 * i * Math.PI) / num_poles);
      opole.translateX(0.8 * width); //horse.translateX(0.8 * width);
      poles.push(opole); //horses.push(horse);
      loader.load("../for_students/teapot.obj", function(suzanne) {
        //suzanne.rotateZ(Math.PI/2);
        console.log(typeof suzanne)
        suzanne.scale.set(0.02, 0.02, 0.02);
        platform_group.add(suzanne);
        suzanne.translateY(1.5);
        suzanne.rotateY((2 * i * Math.PI) / num_poles);
        suzanne.translateX(0.8 * width);
        horses.push(suzanne);
      });
    }
    ...
    this.time = 0;
    this.tick = function(delta, timeOfDay) {
      platform_group.rotateY(0.002 * delta);
      this.time += 1/50;
      for (let i = 0; i < horses.length; i++) {
        horses[i].position.y = Math.sin(this.time)*0.3 + 1
      }
    };
